...
function defaultData() {
  return { tokens: CATALOG.startTokens || 1500, exp: 0, packsOpened: 0, inventory: {}, lastClaim: "" };
}
function getData(u) {
...
function openProfile(username) {
  ...
  document.getElementById("profileAvatar").textContent = realName.charAt(0).toUpperCase();
  document.getElementById("profileName").textContent = realName;
  document.getElementById("profileSubtitle").textContent = "Level " + lvl + (isMe ? " · You" : "");

  const stats = [];
  if (isMe) stats.push(["Tokens", d.tokens.toLocaleString() + " 🪙"]);
  stats.push(["Level", lvl], ["XP", d.exp.toLocaleString()], ["Packs", d.packsOpened || 0], ["Types", types.length], ["Total", total]);
  ...
  if (!types.length) {
    ...
    blooksEl.innerHTML = types.map(n => {
      ...
      const qty = inv[n];
      return '<div class="profile-blook" title="' + n + " ×" + qty + '">' +
        '<img src="' + (b ? b.img : "") + '" alt="" onerror="this.style.display=\'none\'">' +
        (qty > 1 ? '<span class="qty">' + qty + "</span>" : "") + "</div>";
    }).join("");
  }
